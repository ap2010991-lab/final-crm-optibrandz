...
router.get("/", asyncRoute(async (_req, res) => {
  ...
  const [clients, leads, invoices, tasks, serviceOrders, campaigns, calendarItems, users] = await Promise.all([
  ...
  const monthKeys = Array.from({ length: 6 }, (_, index) => new Date(now.getFullYear(), now.getMonth() - 5 + index, 1).toLocaleDateString("en-IN", { month: "short" }));
  ...
  invoices.forEach((invoice) => {
    const key = new Date(invoice.createdAt || invoice.dueDate || Date.now()).toLocaleDateString("en-IN", { month: "short" });
  ...
  res.json({ data: {
    ...
    overdueTasksCount: tasks.filter((task) => new Date(task.dueDate) < now && task.status !== "DONE").length,
    idleLeadsCount: leads.filter((lead) => lead.followUpDate && new Date(lead.followUpDate) < now && !["CONVERTED", "LOST"].includes(lead.status)).length,
