...
const balanceDue = (client) => Math.max(Number(client?.totalValue || 0) - Number(client?.advancePaid || 0), 0);
function validateClientPayments(client) {
  if (Number(client.advancePaid || 0) > Number(client.totalValue || 0)) throw new Error("Advance received cannot be greater than the total contract value.");
}
const AGENCY_WHATSAPP = "+91 8756646053";
...
function Dashboard() {
  ...
  const d = data?.data;
  const cards = [["Active Clients", d?.totalActiveClients, BriefcaseBusiness], ["MRR", money(d?.mrr), CircleDollarSign], ["Deal Value", money(d?.contractedValue), FileText], ["Advances", money(d?.advanceReceived), CheckCircle2], ["Balance Due", money(d?.dealBalanceDue), CircleDollarSign], ["Outstanding", money(d?.totalOutstanding), FileText], ["New Leads", d?.newLeadsThisWeek, Megaphone], ["Collection", `${d?.collectionRate || 0}%`, Gauge]];
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><MiniStat label="Active Services" value={d?.activeServicesCount} detail="Live retainers and projects" /><MiniStat label="Content Review" value={d?.contentInReview} detail="Posts waiting for approval" /><MiniStat label="Campaign Leads" value={d?.campaignLeads} detail="Leads from campaign reports" /><MiniStat label="Conversion Rate" value={`${d?.conversionRate || 0}%`} detail="Converted leads vs pipeline" /></div>
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.95fr]"><div className="panel h-[360px] min-w-0"><div className="flex items-center justify-between"><h2 className="section-title">Revenue Trend</h2><span className="text-xs font-black text-zinc-400">Invoiced vs collected</span></div><ResponsiveContainer width="100%" height={280}><AreaChart data={d?.revenueChart || []}><CartesianGrid strokeDasharray="3 3" stroke="#e5e0d6" /><XAxis dataKey="month" /><YAxis /><Tooltip formatter={(value) => money(value)} /><Area dataKey="invoiced" name="Invoiced" stroke="#ff7a18" fill="#ffe0c2" isAnimationActive={false} /><Area dataKey="collected" name="Collected" stroke="#17836f" fill="#d7f3eb" isAnimationActive={false} /></AreaChart></ResponsiveContainer></div><div className="panel h-[360px] min-w-0"><h2 className="section-title">Lead Funnel</h2><ResponsiveContainer width="100%" height={280}><BarChart data={d?.leadFunnel || []}><CartesianGrid strokeDasharray="3 3" stroke="#e5e0d6" /><XAxis dataKey="status" tickFormatter={(value) => pretty(value).split(" ")[0]} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={pretty} /><Bar dataKey="count" radius={[6, 6, 0, 0]} fill="#ff7a18" isAnimationActive={false} /></BarChart></ResponsiveContainer></div></div>
    <div className="grid gap-5 xl:grid-cols-3"><div className="panel h-80 min-w-0"><h2 className="section-title">Service Mix</h2><ResponsiveContainer width="100%" height={230}><PieChart><Pie data={d?.activeClientsByService || []} dataKey="count" nameKey="service" innerRadius={52} outerRadius={86} paddingAngle={3}>{(d?.activeClientsByService || []).map((entry, index) => <Cell key={entry.service} fill={chartColors[index % chartColors.length]} />)}</Pie><Tooltip formatter={(value, name) => [value, pretty(name)]} /></PieChart></ResponsiveContainer><DashboardLegend items={d?.activeClientsByService || []} colors={chartColors} labelKey="service" /></div><div className="panel h-80 min-w-0"><h2 className="section-title">Invoice Status</h2><ResponsiveContainer width="100%" height={250}><BarChart data={d?.invoiceStatusChart || []}><CartesianGrid strokeDasharray="3 3" stroke="#e5e0d6" /><XAxis dataKey="status" tickFormatter={pretty} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={pretty} /><Bar dataKey="count" radius={[6, 6, 0, 0]} fill="#17836f" isAnimationActive={false} /></BarChart></ResponsiveContainer></div><div className="panel h-80 min-w-0"><h2 className="section-title">Team Load</h2><ResponsiveContainer width="100%" height={250}><BarChart data={d?.teamLoad || []}><CartesianGrid strokeDasharray="3 3" stroke="#e5e0d6" /><XAxis dataKey="name" /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="total" name="Tasks" radius={[6, 6, 0, 0]} fill="#ff7a18" isAnimationActive={false} /><Bar dataKey="overdue" name="Overdue" radius={[6, 6, 0, 0]} fill="#ef4444" isAnimationActive={false} /></BarChart></ResponsiveContainer></div></div>
    <div className="grid gap-5 xl:grid-cols-3"><DashboardList title="Priority Tasks" rows={d?.priorityTasks || []} render={(item) => <><span>{item.title}</span><Badge tone={item.priority}>{pretty(item.priority)}</Badge></>} /><DashboardList title="Top Campaigns" rows={d?.topCampaigns || []} render={(item) => <><span>{pretty(item.platform)}</span><span className="text-xs font-black text-zinc-500">{item.leadsGenerated} leads · CPL {money(item.cpl)}</span></>} /><DashboardList title="Recent Invoices" rows={d?.recentInvoices || []} render={(item) => <><span>{item.invoiceNumber}</span><span className="text-xs font-black text-zinc-500">{money(item.totalAmount)} · {pretty(item.status)}</span></>} /></div>
...
function MiniStat({ label, value, detail }) { return <div className="mini-stat"><div><span>{label}</span><strong>{value || 0}</strong></div><p>{detail}</p></div>; }
function DashboardLegend({ items = [], colors = [], labelKey }) { return <div className="dashboard-legend">{items.map((item, index) => <span key={item[labelKey]}><i style={{ background: colors[index % colors.length] }} />{pretty(item[labelKey])}: {item.count}</span>)}</div>; }
function DashboardList({ title, rows = [], render }) { return <div className="panel"><h2 className="section-title">{title}</h2><div className="mt-3 space-y-2">{rows.length ? rows.map((item, index) => <div key={item.id || item.invoiceNumber || item.platform || index} className="dashboard-list-row">{render(item)}</div>) : <div className="text-sm font-semibold text-zinc-500">No records yet</div>}</div></div>; }
...
function Field({ label, value, onChange, type = "text", options, rows = 1, kind, help, placeholder, min, step }) {
  if (kind === "multi") {
  ...
  return <label className="block">
    ...
    {options ? <select className="input" value={value || ""} onChange={(e) => onChange(e.target.value)}>{options.map((item) => { const option = typeof item === "string" ? { value: item, label: pretty(item) } : item; return <option key={option.value} value={option.value}>{option.label}</option>; })}</select> :
      ...
      <input className="input" type={type} min={min} step={step} placeholder={placeholder} value={value ?? ""} onChange={(e) => onChange(e.target.value)} />}
    {help && <span className="field-help">{help}</span>}
  </label>;
}
...
function Clients() {
  ...
  const clientFields = [
    ...
    { name: "mrr", label: "Monthly Retainer (MRR)", kind: "number", type: "number", min: "0", step: "0.01" }, { name: "totalValue", label: "Total Deal Value", kind: "number", type: "number", min: "0", step: "0.01" },
    { name: "advancePaid", label: "Advance Received", kind: "number", type: "number", min: "0", step: "0.01" },
    { name: "healthScore", label: "Health Score", kind: "number", type: "number", min: "0" }, { name: "renewalDate", label: "Renewal Date", type: "date" }
  ];
  async function saveClient(payload) {
    validateClientPayments(payload);
...
function ClientDetail() {
  ...
  async function saveClient(payload) { validateClientPayments(payload); await api(`/clients/${id}`, { method: "PUT", body: JSON.stringify(payload) }); refreshCRM(); refetch(); }
  return <DetailLayout title={client.businessName} aside={<><Badge tone={client.status}>{pretty(client.status)}</Badge><Info label="Contact" value={`${client.contactPerson} · ${client.phone}`} /><Info label="City" value={client.city} /><Info label="MRR" value={money(client.mrr)} /><Info label="Balance Due" value={money(client.balanceDue ?? balanceDue(client))} /><Info label="Health" value={`${client.healthScore}%`} /><Info label="Renewal" value={date(client.renewalDate)} /><button className="secondary-button w-full" onClick={() => setEditing(true)}><Edit3 size={16} /> Edit Client</button></>}><div className="tabs">{tabs.map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item}</button>)}</div>{tab === "Overview" && <div className="grid gap-4 md:grid-cols-2"><Info label="Industry" value={client.industry} /><Info label="Website" value={client.websiteUrl} /><Info label="Monthly Retainer" value={money(client.mrr)} /><Info label="Total Deal Value" value={money(client.totalValue)} /><Info label="Advance Received" value={money(client.advancePaid)} /><Info label="Balance Due" value={money(client.balanceDue ?? balanceDue(client))} /></div>}{tab === "Services" && <DataTable rows={client.services} columns={["serviceType", "packageName", "monthlyValue", "status"]} />}{tab === "Invoices" && <DataTable rows={client.invoices} columns={["invoiceNumber", "totalAmount", "status", "dueDate"]} />}{tab === "Campaigns" && <DataTable rows={client.campaigns} columns={["platform", "adSpend", "leadsGenerated", "ctr", "cpl"]} />}{tab === "Content" && <DataTable rows={client.calendarItems} columns={["platform", "postType", "scheduledDate", "status"]} />}{tab === "Activity" && <Timeline items={client.activities || []} />}{editing && <EditRecordModal title="Edit Client" initial={{ ...client, services: client.services?.filter((item) => item.status === "ACTIVE").map((item) => item.serviceType), renewalDate: client.renewalDate?.slice?.(0, 10) }} fields={[{ name: "businessName", label: "Business Name" }, { name: "contactPerson", label: "Contact Person" }, { name: "phone", label: "Phone" }, { name: "email", label: "Email" }, { name: "services", label: "Services Chosen", kind: "multi", options: ["SEO", "SMO", "SMM", "GOOGLE_ADS", "META_ADS", "WEBSITE", "GMB", "CONTENT", "GRAPHIC_DESIGN", "YOUTUBE"] }, { name: "city", label: "City" }, { name: "industry", label: "Industry" }, { name: "websiteUrl", label: "Website" }, { name: "status", label: "Status", options: ["ACTIVE", "ONBOARDING", "PAUSED", "CHURNED"] }, { name: "mrr", label: "Monthly Retainer (MRR)", kind: "number", type: "number", min: "0", step: "0.01" }, { name: "totalValue", label: "Total Deal Value", kind: "number", type: "number", min: "0", step: "0.01" }, { name: "advancePaid", label: "Advance Received", kind: "number", type: "number", min: "0", step: "0.01" }, { name: "healthScore", label: "Health Score", kind: "number", type: "number", min: "0" }, { name: "renewalDate", label: "Renewal Date", type: "date" }]} onSubmit={saveClient} onClose={() => setEditing(false)} />}</DetailLayout>;
}
...
function Invoices() {
  ...
  async function saveInvoice(payload) {
    const body = { clientId: payload.clientId, dueDate: new Date(payload.dueDate).toISOString(), gstAmount: payload.gstAmount, paidAmount: payload.paidAmount, status: payload.status, clientPhone: payload.clientPhone, lineItems: [{ description: payload.description, amount: payload.amount }] };
  ...
  function editInvoice(row) { setEditing({ ...row, clientPhone: row.clientPhone || row.client?.phone, description: row.lineItems?.[0]?.description || "Service", amount: row.lineItems?.[0]?.amount || row.amount || 0, dueDate: row.dueDate?.slice?.(0, 10) }); }
  function whatsappInvoiceUrl(row) {
    const phone = normalizePhone(row.clientPhone || row.client?.phone);
    if (!phone) return "";
    const pdfUrl = `${window.location.origin}/api/public/invoices/${row.id}/pdf`;
  ...
  return <div className="panel"><div className="toolbar"><h2 className="section-title">Invoices</h2><button className="primary" onClick={() => setEditing({ clientId: firstClient?.id, clientPhone: firstClient?.phone, description: "Monthly retainer", amount: 0, gstAmount: 0, paidAmount: 0, status: "PENDING" })}><Plus size={16} /> Create Invoice</button></div><DataTable rows={data?.data || []} columns={["invoiceNumber", "client.businessName", "client.phone", "totalAmount", "status", "dueDate"]} action={(row) => <div className="flex flex-wrap gap-2"><button className="table-action" onClick={() => editInvoice(row)}><Edit3 size={14} /> Edit</button><a className="table-action" href={`${window.location.origin}/api/public/invoices/${row.id}/pdf`} target="_blank">PDF</a>{whatsappInvoiceUrl(row) ? <a className="whatsapp-action" href={whatsappInvoiceUrl(row)} target="_blank"><MessageCircle size={14} /> WhatsApp</a> : <button className="whatsapp-action opacity-50" disabled><MessageCircle size={14} /> No Phone</button>}<button className="table-action" onClick={() => markPaid(row)} disabled={row.status === "PAID"}>Mark Paid</button></div>} />{editing && <EditRecordModal title={editing.id ? "Edit Invoice" : "Create Invoice"} initial={editing} fields={[{ name: "clientId", label: "Client", options: (clients?.data || []).map((c) => ({ value: c.id, label: c.businessName })) }, { name: "clientPhone", label: "Client WhatsApp Number" }, { name: "description", label: "Line Item" }, { name: "amount", label: "Amount", kind: "number", type: "number" }, { name: "gstAmount", label: "GST Amount", kind: "number", type: "number" }, { name: "paidAmount", label: "Paid Amount", kind: "number", type: "number" }, { name: "status", label: "Status", options: ["PENDING", "PARTIAL", "PAID", "OVERDUE"] }, { name: "dueDate", label: "Due Date", type: "date" }]} onSubmit={saveInvoice} onClose={() => setEditing(null)} />}</div>;
}
function Campaigns() {
  const [editing, setEditing] = useState(null);
