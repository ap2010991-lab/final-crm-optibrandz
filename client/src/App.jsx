...
function Dashboard() {
  ...
  const d = data?.data;
  const cards = [["Active Clients", d?.totalActiveClients, BriefcaseBusiness], ["MRR", money(d?.mrr), CircleDollarSign], ["Outstanding", money(d?.totalOutstanding), FileText], ["New Leads", d?.newLeadsThisWeek, Megaphone], ["Collection", `${d?.collectionRate || 0}%`, Gauge]];
...
function Clients() {
  ...
  const clientFields = [
    ...
    { name: "mrr", label: "Monthly Retainer (MRR)", kind: "number", type: "number" }, { name: "totalValue", label: "Total Contract Value", kind: "number", type: "number" },
    { name: "healthScore", label: "Health Score", kind: "number", type: "number" }, { name: "renewalDate", label: "Renewal Date", type: "date" }
  ];
